...
| `owns(req)`                    | Whether a request is under the mount path                                                 |
...
Next.js documents `httpServer` as the HTTP server it runs behind. In Next.js 16.3, it's only used to listen for `upgrade` events, so an `EventEmitter` works. Verified with Next.js 16.3.4 in development (hot reload) and production, with a catch-all page.
